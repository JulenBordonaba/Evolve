...
namespace Array2DEditor
{
    public abstract class Array2DEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            scrollPos=GUILayout.BeginScrollView(scrollPos,false,false, GUILayout.MinWidth(100), GUILayout.MaxWidth(1000), GUILayout.ExpandWidth(true), GUILayout.MinHeight(100), GUILayout.MaxHeight(750),GUILayout.ExpandHeight(true));
            //GUILayout.Button("TEst", GUILayout.Width(1000000));
        ...
        private void DisplayGrid(Rect startRect)
        {
            ...
            for (int i = 0; i < gridSize.vector2IntValue.y; i++)
            {
                ...
                for (int j = 0; j < gridSize.vector2IntValue.x; j++)
                {
                    ...
                    EditorGUILayout.LabelField("Bloque [" + (i+1).ToString()+ "/" + (j+1).ToString() + "]", GUILayout.Width(100));
                    EditorGUILayout.PropertyField( row.GetArrayElementAtIndex(j), GUIContent.none, true, GUILayout.MinWidth(150), GUILayout.MaxWidth(300));
                    cellPosition.x += CellWidth + margin;
                }
                EditorGUILayout.EndHorizontal();
