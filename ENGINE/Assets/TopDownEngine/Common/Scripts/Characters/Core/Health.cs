...
namespace MoreMountains.TopDownEngine
{
	...
	public class Health : MonoBehaviour
	{
		...
		public virtual void Damage(int damage,GameObject instigator, float flickerDuration, float invincibilityDuration)
		{
			...
			if ((CurrentHealth <= 0) && (stats.initialHealth != 0))
			{
				return;
			}
		...
		public virtual void GetHealth(int health,GameObject instigator)
		{
			...
			CurrentHealth = Mathf.Min (CurrentHealth + health, stats.baseMaximumHealth);
			UpdateHealthBar(true);
		}
	    ...
	    {
			CurrentHealth = stats.baseMaximumHealth;
			UpdateHealthBar (false);
        }	
        ...
        protected virtual void UpdateHealthBar(bool show)
	    {
	    	if (_healthBar != null)
	    	{
				_healthBar.UpdateBar(CurrentHealth, 0f, stats.baseMaximumHealth, show);
	    	}
	    	...
	    	if (_character != null)
	    	{
	    		if (_character.CharacterType == Character.CharacterTypes.Player)
	    		{
					...
					{
						GUIManager.Instance.UpdateHealthBar(CurrentHealth, 0f, stats.baseMaximumHealth, _character.PlayerID);
					}
	    		}
	    	}
	    }
